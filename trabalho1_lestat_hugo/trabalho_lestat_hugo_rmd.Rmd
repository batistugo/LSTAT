---
title: "LESTAT- Óbitos por Neoplasias Infantis"
...
# Introdução
Para a construção das visualizações foi coletado óbitos por Neoplasias Malignas no site do DATASUS usando a CID-10 de C48 até C80. Para o número da população, foi utilizada projeções da população do DATASUS. Tendo o número de óbitos e a projeção da população, foi então construído a taxa de óbito para cada 100 mil habitantes. Os dados estão no período de 2000 até 2020, e serão feitas análises comparando todas as regiões geográficas e cada estado dentro de cada região geográfica.

## Gerando os dados para construção das descritivas
...
## Contruindo os códigos para os gráficos

Para melhor auxiliar e deixar os gráficos mais limpos, segue tabela de legenda:

```{r}
proj <- as.data.table(read_excel("dados/projecao_pop.xlsx", skip = 3L))
proj <- proj[!is.na(cod_univ)]
proj <- proj[, 1L:2L]
names(proj) <- c("Nome", "Codigo")
proj
```
