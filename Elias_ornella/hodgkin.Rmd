...
```{r fig.cap = "Figura 2: Taxa de mortalidade por Linfoma de Hodgkin em crianças e adolescentes no Brasil, por ano e região."}
t <- d %>% 
  group_by(ano,regiao,base) %>% 
  summarise(n_total = sum(n)) 

t1 <- t %>% filter(base=="hodgkin" | base=="neoplasias")

t2 <- t %>% filter(base=="populacao") %>% rename(n_pop = n_total) %>% select(-base)

dados2 <- full_join(t1,t2,by=c("ano","regiao")) %>% mutate(taxa=n_total/n_pop*100000)
...
```{r fig.cap = "Figura 3: Taxa de mortalidade por Linfoma de Hodgkin em crianças e adolescentes no Brasil, por faixa etária e região."}
